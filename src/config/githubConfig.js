...
export function validateConfig(config) {
  ...
  // Validate arrays
  validated.filters.includeTopics = Array.isArray(validated.filters.includeTopics) 
    ? validated.filters.includeTopics : [];
  validated.filters.excludeTopics = Array.isArray(validated.filters.excludeTopics) 
    ? validated.filters.excludeTopics : [];
  validated.filters.includeLanguages = Array.isArray(validated.filters.includeLanguages) 
    ? validated.filters.includeLanguages : [];
  validated.filters.excludeLanguages = Array.isArray(validated.filters.excludeLanguages) 
    ? validated.filters.excludeLanguages : [];
  
  // Validate sorting options
  const validSortOptions = ['stars', 'created', 'updated', 'name', 'size', 'forks'];
  if (!validSortOptions.includes(validated.sorting.by)) {
    validated.sorting.by = 'updated';
  }
  if (!['asc', 'desc'].includes(validated.sorting.order)) {
    validated.sorting.order = 'desc';
  }
